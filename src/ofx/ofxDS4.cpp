...
void ofxDS4::setup(ConectionMode mode) {
	...
	if (!_hidDevice) {
		std::cerr << "Unable to find a Gyro" << std::endl;
	}
		...
		if (_resultFlag < 0) { std::cerr << "unable to sent report" << std::endl; }
...
void ofxDS4::_updateHID()
{
	...
	while (_resultFlag == 0) {
		...
		if (_resultFlag < 0) std::cerr << "Unable to read data from the Gyro" << std::endl;
	}
}
